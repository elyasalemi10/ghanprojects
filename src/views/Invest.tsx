...
export default function Invest() {
  ...
  const handleRequestPack = (title: string) => {
    router.push(`/book-consultation?message=${encodeURIComponent(`I am interested in an investment pack opportunity for ${title}`)}`);
  };
    ...
    <div className="bg-background">
      ...
      <section className="relative py-32 lg:py-48 px-6 lg:px-12 bg-primary text-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            <div className="inline-block px-4 py-1 bg-accent/20 border border-accent/30 text-accent text-[10px] font-bold tracking-widest mb-8">
            ...
            {opportunities.map((opp, i) => (
              <FadeInWhenVisible key={i} delay={i * 0.1}>
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-10 hover:bg-white/10 transition-all duration-500 group h-full flex flex-col">
                  <div className="text-[10px] tracking-widest font-bold text-accent mb-4 px-2 py-1 bg-accent/10 inline-block self-start">
                    {opp.status}
                  </div>
                  ...
                  <Button
                    ...
                    className="w-full rounded-none bg-accent hover:bg-accent/90 text-white py-6 font-heading font-bold tracking-wider text-xs mt-auto"
                  >
          ...
          <FadeInWhenVisible>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button
                asChild
                size="lg"
                className="rounded-none px-12 py-8 text-lg font-heading font-bold tracking-wider bg-accent text-white hover:bg-accent/90"
              >
                <Link href="/book-consultation">Speak With An Advisor</Link>
              </Button>
              <Button
                asChild
                size="lg"
                variant="outline"
                className="rounded-none px-12 py-8 text-lg font-heading font-bold tracking-wider border-white text-primary hover:bg-white hover:text-primary"
              >
                <Link href="/portfolio">
