...
export default function Home({ insights }: { insights: BlogPost[] }) {
  ...
  const handleJoinNetwork = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`/api/investor-network`, {
        method: 'POST',
        ...
        <div className="relative z-20 max-w-4xl mx-auto lg:mx-0 text-white pt-16 md:pt-0">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-8">
              <Button asChild size="lg" className="rounded-none px-6 sm:px-8 py-5 sm:py-7 text-base sm:text-lg font-heading font-bold tracking-wider bg-accent text-white hover:bg-accent/90">
                <Link href="/book-consultation">Book Consultation</Link>
              </Button>
              <Button asChild size="lg" className="rounded-none px-6 sm:px-8 py-5 sm:py-7 text-base sm:text-lg font-heading font-bold tracking-wider bg-white text-primary hover:bg-white/90">
                <Link href="/portfolio">View Portfolio</Link>
              </Button>
            </div>
            <div className="flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-2 text-xs sm:text-sm text-white/60 font-medium tracking-[0.15em] sm:tracking-[0.2em]">
              <span>Advisory</span>
            ...
            {stats.map((stat, i) => (
              <FadeInWhenVisible key={i} delay={i * 0.1}>
                <div className="text-center lg:text-left space-y-2">
                  ...
                  <div className="text-sm tracking-widest text-muted-foreground font-semibold">{stat.label}</div>
                </div>
              </FadeInWhenVisible>
            ))}
          </div>
            ...
            {services.map((service, i) => (
              <FadeInWhenVisible key={i} delay={i * 0.1}>
                <div className="group bg-background p-10 border hover:border-accent transition-all duration-500 h-full flex flex-col shadow-sm hover:shadow-xl">
                  ...
                  <Link href="/services" className="inline-flex items-center gap-2 text-primary font-bold tracking-widest text-xs hover:text-accent transition-colors">
                ...
                <div className="group cursor-pointer">
                  ...
                  <div className="flex gap-2 mb-4">
                    {project.tags.map(tag => (
                      <span key={tag} className="text-[10px] tracking-widest font-bold px-2 py-1 bg-secondary text-primary">{tag}</span>
                    ))}
                  </div>
                  ...
                  <Button asChild variant="ghost" className="p-0 hover:bg-transparent text-primary hover:text-accent font-bold tracking-widest text-xs gap-2">
                    <Link href="/portfolio">View Project <ChevronRight size={16} /></Link>
                  </Button>
                </div>
              </FadeInWhenVisible>
            ))}
            ...
            {opportunities.map((opp, i) => (
              <FadeInWhenVisible key={i} delay={i * 0.1}>
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-10 hover:bg-white/10 transition-all duration-500 group">
                  <div className="text-[10px] tracking-widest font-bold text-accent mb-4 px-2 py-1 bg-accent/10 inline-block">{opp.status}</div>
                  ...
                  <Button 
                    onClick={() => router.push(`/book-consultation?message=${encodeURIComponent(`I am interested in an investment pack opportunity for ${opp.title}`)}`)}
                    className="w-full rounded-none bg-accent hover:bg-accent/90 text-white py-4 sm:py-6 font-heading font-bold tracking-wider text-[10px] sm:text-xs whitespace-nowrap"
                  >
                    Request Info Pack
                  </Button>
                </div>
      ...
      <section className="py-32 bg-background px-6 lg:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-20 text-center">
            <Button asChild size="lg" className="rounded-none px-10 py-7 font-heading font-bold tracking-wider text-xs bg-primary text-white">
              <Link href="/book-consultation">Book a Strategy Call</Link>
            </Button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20">
          ...
          <div>
            <div className="flex justify-between items-end mb-12">
              ...
              <Link href="/insights" className="text-xs tracking-widest font-bold text-accent hover:text-primary transition-colors">View All</Link>
            </div>
                ...
                <FadeInWhenVisible key={article.slug} delay={i * 0.1}>
                  ...
                  >
                    <div className="w-24 h-24 bg-secondary shrink-0 hidden sm:block overflow-hidden">
                      <img src={article.thumbnail || '/images/property-analysis.webp'} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt={article.thumbnail_alt || article.title} />
                    </div>
                    <div>
          ...
          <div>
            <div className="flex justify-between items-end mb-12">
              ...
              <Link href="/resources" className="text-xs tracking-widest font-bold text-accent hover:text-primary transition-colors">All Resources</Link>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-4 sm:gap-6">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] tracking-widest text-white/50">{res.type}</span>
                        </div>
                      </div>
              ...
              <div className="bg-secondary/30 p-6 sm:p-10 lg:p-14 border border-secondary shadow-2xl">
                <form onSubmit={handleJoinNetwork} className="space-y-4 sm:space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] tracking-widest font-bold text-primary">Email <span className="text-muted-foreground normal-case">(or phone)</span></label>
                      <input 
                        name="email" 
                        type="email" 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] tracking-widest font-bold text-primary">Phone <span className="text-muted-foreground normal-case">(or email)</span></label>
                      <input 
                        name="phone" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] tracking-widest font-bold text-primary">Budget Range</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] tracking-widest font-bold text-primary">Investment Interest</label>
                  ...
                  <Button type="submit" className="w-full rounded-none bg-primary hover:bg-primary/95 text-white py-5 sm:py-8 font-heading font-bold tracking-wider text-xs sm:text-sm mt-4">
                    Join Network
                  </Button>
              ...
              <FadeInWhenVisible key={i} delay={i * 0.1}>
                <div className="bg-background p-10 border shadow-sm relative italic leading-relaxed text-muted-foreground">
                  ...
                  <div className="not-italic">
                    <h4 className="font-heading font-bold text-primary tracking-wider">{t.name}</h4>
                    <span className="text-[10px] tracking-widest text-accent font-bold">{t.role}</span>
                  </div>
                </div>
              </FadeInWhenVisible>
            ))}
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          <FadeInWhenVisible>
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 px-4">
              <Button asChild size="lg" className="rounded-none px-8 sm:px-12 py-6 sm:py-8 text-sm sm:text-lg font-heading font-bold tracking-wider bg-accent text-white hover:bg-accent/90">
                <Link href="/book-consultation">Contact Us</Link>
              </Button>
              <Button asChild size="lg" className="rounded-none px-8 sm:px-12 py-6 sm:py-8 text-sm sm:text-lg font-heading font-bold tracking-wider bg-white text-primary hover:bg-white/90">
